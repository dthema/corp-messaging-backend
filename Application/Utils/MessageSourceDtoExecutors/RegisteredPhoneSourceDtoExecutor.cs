...
public class RegisteredPhoneSourceDtoExecutor : IMessageSourceDtoExecutor
{
    private IMessageSourceDtoExecutor? _nextFactory;
    private readonly IMessageSourceCommand _command;

    public RegisteredPhoneSourceDtoExecutor(IMessageSourceCommand command)
    {
        _command = command;
    }
    ...
    {
        if (messageSourceDto is PhoneSourceDto phoneSourceDto)
            return await _command.Execute(new PhoneSource
            {
                Id = phoneSourceDto.Id,
