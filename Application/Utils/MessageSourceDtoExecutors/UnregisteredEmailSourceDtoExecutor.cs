...
public class UnregisteredEmailSourceDtoExecutor : IMessageSourceDtoExecutor
{
    private IMessageSourceDtoExecutor? _nextFactory;
    private readonly IMessageSourceCommand _command;

    public UnregisteredEmailSourceDtoExecutor(IMessageSourceCommand command)
    {
        _command = command;
    }
    ...
    {
        if (messageSourceDto is UnregisteredEmailSourceDto emailSourceDto)
