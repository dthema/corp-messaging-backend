...
public class UnregisteredMessengerSourceDtoExecutor : IMessageSourceDtoExecutor
{
    private IMessageSourceDtoExecutor? _nextFactory;
    private readonly IMessageSourceCommand _command;

    public UnregisteredMessengerSourceDtoExecutor(IMessageSourceCommand command)
    {
        _command = command;
    }
    ...
    {
        if (messageSourceDto is UnregisteredMessengerSourceDto messengerSourceDto)
